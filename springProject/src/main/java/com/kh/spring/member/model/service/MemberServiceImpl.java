...
public class MemberServiceImpl implements MemberService {
	...
	public Member login(Member member){
		...
		// 입력 받은 비밀번호와, DB에 저장되어있는 암호화된 비밀번호가 
		// 일치하는지 확인하는 작업이 필요함.
	...
	public int updatePwd(Member loginMember, String newPwd1) {
		...
		// -> bcrypt 암호화을 사용하고 있어서 DB에서 비밀번호 일치 여부 판단이 안됨.
	...
	public int deleteMember(Member loginMember) {
		// bcrypt 암호화 때문에 DB에서 비밀번호 일치 여부를 판단 할 수 없으므로
		// DB에 저장된 비밀번호 조회하여 비밀번호 일치 여부 확인하기
		String savePwd = memberDAO.selectPwd(loginMember.getMemberNo());
		
		int result = 0;
		
		// 조회가 되지 않았을 경우를 대비한 null 처리
		if(savePwd != null) {
			
			// 저장된 비밀번호와 입력받은 현재 비밀번호가 일치하는지 확인
			if(bcPwd.matches(loginMember.getMemberPwd(), savePwd)) {
				result =  memberDAO.deleteMember(loginMember.getMemberNo());
			}
		}
		return result;
	}
