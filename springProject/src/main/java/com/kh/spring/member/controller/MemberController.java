...
// -> Session에서 "loginMember"라는 key를 가지는 속성의 값을 얻어와라
...
public class MemberController {
	 ...
	 *  
	 *  -> 만약 input태그에 값(value)이 비어있다면 ""(빈 문자열)이 넘어옴
	 *  
	 *  단, 파라미터 중 일치하는 name 속성값이 없을 경우
	 *   HTTP Status 400 - Bad Request 발생
	 *   
	 *  [사용법 2] : 속성 추가
	 *  value : 전달 받은 input태그의 name 속성 값
	...
	public String loginAction(Member member, Model model, RedirectAttributes rdAttr,
		...
		//  saveId : check box가 체크가 되어진 경우 별도의 value 속성이 없으면 "on"이라는 문자열이 전달됨.
		//   			check box가 체크가 안되어진 경우는 null
	...
	public String logout(SessionStatus status) {
		...
		// SessionStatus.setComplte() 메소드를 사용해야함.
	...
	public String etcException(Exception e, Model model, HttpServletRequest reuqest) {
		e.printStackTrace();
		
		System.out.println(reuqest.getRequestURI()); // 예외가 발생한 요청 주소
